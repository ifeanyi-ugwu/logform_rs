...
fn old_json_format_benchmark(c: &mut Criterion) {
    let formatter = logform::legacy_format::json(); // Old version of JSON formatting
...
fn new_json_format_benchmark(c: &mut Criterion) {
    let formatter = logform::json(); // New version of JSON formatting
...
fn old_timestamp_format_benchmark(c: &mut Criterion) {
    let formatter = logform::legacy_format::timestamp(); // Old timestamp formatter
...
fn new_timestamp_format_benchmark(c: &mut Criterion) {
    let formatter = logform::timestamp(); // New timestamp formatter
